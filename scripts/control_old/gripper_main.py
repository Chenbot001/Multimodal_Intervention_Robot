...
from src.core.config_loader import GRIPPER_CONFIG, SYSTEM_CONFIG, DISPLAY_CONFIG, ADAPTIVE_GRIPPING_CONFIG, RECORDING_CONFIG, SAFETY_CONFIG
...
class SystemState:
    ...
    def __init__(self):
        ...
        self.object_diameter_mm = SYSTEM_CONFIG["initial_diameter_mm"]
...
class GripperControlSystem:
    ...
    def initialize(self):
        ...
        if self.state.hardware.gripper.connected:
            print(f"Initializing gripper to default open position ({GRIPPER_CONFIG['motor']['default_open_percent']}% closed)...")
            self.state.hardware.gripper.move_to_percentage(GRIPPER_CONFIG["motor"]["default_open_percent"])
            time.sleep(2)
    ...
    def update_display(self):
        ...
        print(f"  Target Steps:   {GRIPPER_CONFIG['stepper']['fixed_target_steps']:>6}")
    ...
    def on_press(self, key):
        ...
        try:
            ...
            # --- Stepper Motor Controls (Gripper Rotation) ---
                ...
                self.state.hardware.stepper.speed = GRIPPER_CONFIG["stepper"]["fixed_motor_speed"]
                
                print(f"🚀 Starting CCW step move: Target steps = {GRIPPER_CONFIG['stepper']['fixed_target_steps']}")
                
                # Step-based control for all diameters
                self.state.hardware.stepper.send_step_move_command(
                    GRIPPER_CONFIG["stepper"]["fixed_target_steps"], 'ccw'
                )
                ...
                print(f"🚀 Starting CW step move: Target steps = {GRIPPER_CONFIG['stepper']['fixed_target_steps']}")
                
                # Step-based control for all diameters
                self.state.hardware.stepper.send_step_move_command(
                    GRIPPER_CONFIG["stepper"]["fixed_target_steps"], 'cw'
                )
            ...
            # --- Sensor Controls ---
                ...
                if self.state.sensors.visuotactile.connected:
                    ...
                    SAFETY_CONFIG["enabled"] = not SAFETY_CONFIG["enabled"]
                    status = "enabled" if SAFETY_CONFIG["enabled"] else "disabled"
            ...
            # --- Utility Commands ---
                ...
                if gripper_closure <= GRIPPER_CONFIG["motor"]["default_open_percent"]:  # Allow homing when at default open position or more open
                    if self.state.hardware.stepper.connected:
                        ...
                        homing_speed = GRIPPER_CONFIG["stepper"]["homing_speed"]
                    ...
                    print(f"⚠ Motor homing BLOCKED: Gripper must be at default open position (≤{GRIPPER_CONFIG['motor']['default_open_percent']}% closed, currently {gripper_closure:.1f}% closed)")
